/**
 * Proves the cu disk extension's BUILT renderer bundle
 * (extensions/cu/dist/renderer.js), when imported and activated with the host's
 * React, renders the Fleet panel WITHOUT an "Invalid hook call" and with lucide
 * icons present — i.e. react/jsx-runtime correctly resolve to the single injected
 * React, and lucide-react is bundled in.
 *
 ...
 *
 * The host stub returns `{ running: false }` for `daemonStatus` (an OBJECT — the
 * panel reads `.running`), which lands the panel in its daemon-down state: a
 * clean first render with no thrown capability, exercising the full hook + JSX +
 * lucide path that a second React would crash.
 */
...
function makeHost(): ModuleHost {
  return {
    ...
    call: async (capability: string) => {
      // daemonStatus must be an OBJECT with `running` (not an array) so the
      // panel paints its daemon-down state cleanly. Everything else resolves
      // empty/benign.
      if (capability === 'daemonStatus') return { running: false } as unknown;
      if (capability === 'dashboard') return {} as unknown;
      return [] as unknown;
    },
...
describe.skipIf(!built)('cu built renderer bundle (blob-import contract)', () => {
  it('activates with host React and renders the panel — no second-React hook crash, lucide bundled', async () => {
    ...
    // With the stub's daemonStatus → { running: false }, the panel lands on its
    // daemon-down gate — a clean render exercising hooks + JSX + lucide.
    expect(html).toContain('cu-panel');
    expect(html).toContain('<svg'); // lucide icons render to inline SVG
    expect(html).toContain('Claude Unleashed');

    // The ported contributions behave.
    const cmds = activated.commands!(host);
    expect(cmds[0]?.id).toBe('refresh-fleet');
    // No running sessions in the stub → no badge.
    expect(activated.navBadge!(host)).toBe(null);
  });
});
