...
/**
 * Library-mode build for the Claude Unleashed (cu) DISK extension. Two targets
 * (selected by BUILD_TARGET), each externalizing a different set, both emitting
 * a single self-contained ESM file whose name matches `entry` in extension.json.
 *
 * RENDERER (same headline constraint as the gus-disk dogfood):
 ...
 *       externalized). The panel imports a handful of icons; they ship in the
 *       artifact.
 ...
 *   cu-main imports neither (pure JS + ctx capabilities), so the rest bundles
 *   cleanly.
 */
    ...
    ? {
        build: {
          ...
          lib: {
            ...
            formats: ['es'],
            // .mjs (not .js) so the host child's dynamic import() treats it as
            // ESM — a .js entry loads as CommonJS and chokes on `export`.
            fileName: () => 'main.mjs'
          },
